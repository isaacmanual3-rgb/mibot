...
class Cell:
    ...
    def boc_b64(self):
        """
        Serializar a BOC base64 — formato estándar TON.

          magic(4) | flags(1) | offset_sz(1) | cells_n(1) | roots_n(1) |
          absent(1) | total_sz(2 bytes) | root_idx(1 byte)

          flags      = 0x01  → ref_byte_size = 1
          offset_sz  = 0x02  → total_cells_size en 2 bytes
          root_index = 0     → la raíz va PRIMERA (pre-order)

        Orden PRE-ORDER: el padre va ANTES que sus hijos.
        Así cada ref apunta a un índice MAYOR que el padre,
        que es lo que TON exige.
          ext[0] -> signed[1] -> body[2] -> int_msg[3] -> cmt[4]  ✓
        """
        cells = []
        ...
        hdr = (
            ...
            + (0).to_bytes(1, 'big')      # root_index = 0 (raíz va primera)
        )
        return base64.b64encode(hdr + payload).decode()

    def _collect(self, out, seen):
        """
        PRE-ORDER: el padre se agrega PRIMERO, luego sus hijos.
        La raíz queda en índice 0, las hojas en índices mayores.
        TON exige: ref.index > parent.index (hijos siempre después).
        """
        if id(self) in seen:
            return
        seen.add(id(self))
        out.append(self)           # padre primero → índice bajo
        for r in self.refs:
            r._collect(out, seen)  # hijos después → índices altos
