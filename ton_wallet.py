"""
ton_wallet.py — v7 FIXED
Envío automático TON wallet-v4r2.
Sin dependencias externas. Solo: stdlib + cryptography + requests.

FIXES vs v6:
  1. BOC header corregido:
       - offset_byte_size = 2  (no 1 — causa base64 de longitud inválida / header corrupto)
       - total_cells_size en 2 bytes (no 1)
       - root_index = n-1      (la root va AL FINAL de la lista, no al inicio)
  2. _collect() vuelve al orden correcto: hijos primero, root al final (post-order)
     → Las refs SIEMPRE apuntan a índices MAYORES que el padre = válido para TON
  3. friendly_to_raw valida longitud del hex hash (64 chars)
  4. Se mantiene la lógica completa de v6 (seqno, memo, etc.)
"""
...
class Cell:
    ...
    def boc_b64(self):
        """
        Serializar a BOC base64 — formato estándar TON verificado:

          magic(4) | flags(1) | offset_sz(1) | cells_n(1) | roots_n(1) |
          absent(1) | total_sz(offset_sz bytes) | root_idx(ref_sz bytes)

          flags       = 0x01  → ref_byte_size = 1
          offset_sz   = 0x02  → total_cells_size en 2 bytes  ← CRÍTICO
          root_index  = n-1   → la celda raíz va AL FINAL    ← CRÍTICO

        Orden de serialización: POST-ORDER (hijos antes que padres).
        Así las refs apuntan a índices > índice del padre, que es lo
        que el nodo TON espera (reference #X must point to cell with
        higher index).
        """
        cells = []
        ...
        blobs = []
        for c in cells:
            # ref indices como 1 byte cada uno (ref_byte_size=1)
        ...
        hdr = (
            ...
            + bytes([0x02])               # offset_byte_size = 2   ← FIX
            + n.to_bytes(1, 'big')        # cells_num
            + (1).to_bytes(1, 'big')      # roots_num = 1
            + (0).to_bytes(1, 'big')      # absent_num = 0
            + total.to_bytes(2, 'big')    # total_cells_size (2 bytes)  ← FIX
            + (n - 1).to_bytes(1, 'big')  # root_index = n-1  ← FIX
        )
        return base64.b64encode(hdr + payload).decode()

    def _collect(self, out, seen):
        """
        Post-order: hijos antes que padres.
        La celda raíz queda en la última posición (índice n-1).
        Así todas las referencias apuntan a índices mayores = válido en TON.
        """
        if id(self) in seen:
            return
        seen.add(id(self))
        for r in self.refs:
            r._collect(out, seen)   # primero los hijos
        out.append(self)            # root al final  ← FIX (v6 lo tenía al revés)
...
def friendly_to_raw(addr):
    ...
    Parsea CUALQUIER formato de dirección TON:
      ...
      "EQD2NmD_..."      → base64url friendly
      "UQBkIKoV..."      → base64url friendly
