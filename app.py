...
def _main_keyboard(user_id, lang='es'):
    ...
    webapp_url = f"{base_url}?user_id={user_id}" if base_url else None
...
def _welcome_text(name, lang='es', verified=False):
    ...
    msgs = {
      ...
      'en': f"⛏️ <b>Welcome, {safe}!</b>\n\n💎 <b>{_BOT_TITLE}</b> — mine TON in real time\n\n⚡ Active hashrate 24 hours a day\n✅ Complete tasks and earn extra rewards\n👥 Invite friends and collect automatic commissions\n💰 Withdraw your earnings in <b>TON</b>\n\n👇 Open the app and start mining now:",
      'pt': f"⛏️ <b>Bem-vindo(a), {safe}!</b>\n\n💎 <b>{_BOT_TITLE}</b> — mine TON em tempo real\n\n⚡ Hashrate ativo 24 horas por dia\n✅ Complete tarefas e ganhe recompensas extras\n👥 Convide amigos e receba comissões automáticas\n💰 Saque seus ganhos em <b>TON</b>\n\n👇 Abra o app e comece a minerar agora:",
      'fr': f"⛏️ <b>Bienvenue, {safe}!</b>\n\n💎 <b>{_BOT_TITLE}</b> — minez du TON en temps réel\n\n⚡ Hashrate actif 24h/24\n✅ Complétez des tâches et gagnez des récompenses\n👥 Invitez des amis et percevez des commissions\n💰 Retirez vos gains en <b>TON</b>\n\n👇 Ouvrez l'app et commencez à miner maintenant:",
    }
...
def _handle_start(msg):
    ...
    if not ok:
        try:
            ...
            logger.error(f"Error enviando join_needed a {user_id}: {e}")
        return

    try:
        keyboard = _main_keyboard(user_id, lang)
    except Exception as e:
    ...
    try:
        ...
        logger.error(f"Error enviando bienvenida a {user_id}: {e}")
